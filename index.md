...
xafty_network$mtcars$link(get_mtcars())

# Instead of passing data to `add_power_to_weight`, we register a dependency using query()
xafty_network$mtcars$link(add_power_to_weight(mtcars = query(mtcars = c("hp", "wt"))))
...
Instead of passing data to each function, we use
[`xafty::query()`](https://davidcrone.github.io/xafty/reference/query.md)
to declare which input variables are needed.
...
- Programming with [states](https://github.com/andybega/states), passed
  alongside queries via
...
- Inspection tools, such as
  ...
  to view the internal dependency graph.
- Debugging tools, such as
  [`xafty::to_script`](https://davidcrone.github.io/xafty/reference/to_script.md),
  which allows you to write the entire pipeline as a linear script
...
- **Added in 0.3.0:** ~~**A flexible filtering mechanism** using a
  ...
  clause, providing SQL-style filtering~~
- **Added in 0.3.2; 0.3.6:** **Developer tooling**, such as
  visualizations and ~~printable summaries~~ of network components and
  ~~writable pipelines~~, to make building and debugging with xafty
  easier
- **Robust testing and bug fixing** to ensure reliability at scale
- **Removed due to change in scope and vision:** ~~**Support for lossy
  transformations** (e.g., `dplyr::group_by() / summarise()`),
  integrated seamlessly into a network structure~~
