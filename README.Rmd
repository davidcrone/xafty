...
**New Update:** xafty now supports the rule: __##!!regexcolumns__ which matches columns in the check table via a regular expression
and applies the rules of that column to every matched column. 
...
Checks for empty (NA) values or values provided below the rule that stand for missing values. Use this check when a column should not have any empty or missing values. Consequently, NA values must be correctly identified before being able to use this check correctly.
