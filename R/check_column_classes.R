...
check_column_classes <- function(check_table, validity_table) {
  ...
  xafty_syntax <- "##!!"
  possible_classes <- c("text", "date", "number")
  ...
  for (i in col_names_check_table) {
    ...
    if (i %in% colnames(validity_table)) {
      ...
      list_result[[i]] <- switch (xafty_data_type,
                                  ...
                                  "##!!date" = lubridate::is.Date(check_table[[i]]),
  ...
  if (all(result_unlisted)) {
    ...
    message <- paste0("Rule Broken: Column Classes. Following columns have the wrong class type: ", wrong_classes_collapsed)
  }

  data.frame("Check" = "Column Classes", "Check_Result" = result, "Message" = message)
