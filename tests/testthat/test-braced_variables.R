...
test_that("xafty states are correctly identified", {
  test_values <- c("normal_character", "{xafty_state}", "{another.xafty_state}", "{inval{_name}", "{}",
                   "name.{variable}", "name_{variable}", "name{variable}", "name.variable", "[name.variable]", "[year.{month}]",
                   "[year.{}]")
  test_logical <- vapply(test_values, contains_state, logical(1), USE.NAMES = FALSE)
  expect_logical <- c(FALSE, TRUE, TRUE, TRUE, FALSE, TRUE, TRUE, TRUE, FALSE, FALSE, TRUE, FALSE)
  expect_identical(test_logical, expect_logical)
})

test_that("xafty objects are correctly identified", {
  test_characters <- c("normal_character", "[xafty_object]", "[another.xafty_state]", "[inval{_name]", "[]", "[year.{month}]")
  test_logical <- is_squared_variable(test_characters)
  expect_logical <- c(FALSE, TRUE, TRUE, FALSE, FALSE, TRUE)
  expect_identical(test_logical, expect_logical)
})

test_that("squared variables are retrieved from the squared brackets", {
  test_characters <- c("[xafty_object]", "[another.xafty_state]", "[inval[_name]", "[year.{month}]")
  test_variables <- vapply(test_characters, get_squared_variable, FUN.VALUE = character(1), USE.NAMES = FALSE)
  expected_characters <- c("xafty_object", "another.xafty_state", "inval[_name", "year.{month}")
  expect_identical(test_variables, expected_characters)
})
