...
test_that("curly variables are retrieved from the curly brackets", {
  test_characters <- c("{xafty_state}", "{another.xafty_state}", "{inval{_name}")
  test_variables <- vapply(test_characters, get_braced_variable, FUN.VALUE = character(1), USE.NAMES = FALSE)
  expected_characters <- c("xafty_state", "another.xafty_state", "inval{_name")
  expect_identical(test_variables, expected_characters)
})

test_that("xafty states are correctly identified", {
  test_logical <- vapply(c("normal_character", "{xafty_state}", "{another.xafty_state}", "{inval{_name}", "{}"),
                         is_xafty_state_variable, logical(1), USE.NAMES = FALSE)
