...
In a xafty network pipeline, however, grouping must be declared explicitly. Because nodes are independent and assembled dynamically, grouping cannot be embedded inline within each function. Instead, xafty lets you define grouped execution contexts at the project level using **entry** and **exit** hooks.
